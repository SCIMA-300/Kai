...
class PolarGraph {
  ...
  void createBase(){
    ...
    commonSettings(b);
    b.strokeWeight(w);
    for (int i = 0; i < P.length; i++){                  //Now we'll assign hue depending on group 
      int eHue = getElement(i).getGroup();           //this the group value
      --groupC[eHue];                              //substract one from its counter
      //this may seem tricky or complex, but it's easy:
      //divide the spectrum of hue by number of groups,divide the 75% of spectrum of brightness by number of group elements and the other 25% 
      color strokeColor = color(cR / (nG + 1) * eHue, cR75,(cR75 / groupN[eHue] * groupC[eHue]) + cR25);  
      b.stroke(strokeColor, cR75);
      
      float n = i < P.length-1?getElement(i + 1).getAng():TWO_PI;  //jumping over an exception
  ...
  void creaConnect () {                 //display selected connections
    for (int i=0; i<P.length; i++){
      if (P[i].getOn()){                //if element is selected
        doConnections(i,#00ffff);       //do the job
      }
    }
  }
